...
class WeChatBot:
    ...
    def _generate_reply(self, msg: dict, answers: dict) -> str | None:
        ...
        emotion = answers.get("emotion", {}).get("choice", "开心")
        my_emotion = answers.get("my_emotion", {}).get("choice", "开心")
        urgency = answers.get("urgency", {}).get("score", 1.0)
        is_group = bool(msg.get("is_group", False))
        ...
        user = (
            ...
            f"辅助判断：对方意图≈{INTENT_LABELS.get(intent, intent)}，对方情绪：{emotion}，"
            f"你现在的情绪：{my_emotion}，紧急度 {urgency:.0f}/5\n\n"
